...
from plots import win_accuracy_scatter
import pandas as pd
import re
...
for dataset in datasets:
    ...
    if group_name not in win_accuracies:
        win_accuracies[group_name] = []

    df = pd.read_csv(f"tests/data/{dataset}")
    num_games[group_name].append(len(df))
    engine = RatingEngine.from_dataframe(df)

    win_accuracy = engine.win_accuracy()
    win_accuracies[group_name].append((win_accuracy, dataset))

fig = win_accuracy_scatter(num_games, win_accuracies)
fig.update_layout(title_text="Win Accuracies")
fig.update_xaxes(title_text="Number of Matches")
fig.update_yaxes(title_text="Accuracy Rate")
# fig.show()
print("writing...")
fig.write_image("tests/plots/win_accuracies.png", scale=3)
print("wrote!")